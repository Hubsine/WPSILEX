...
function create_sellegance_brands_table()
{
	global $wpdb;
	
	$sql = "CREATE TABLE IF NOT EXISTS `".$wpdb->prefix."sellegance_brands` (
	...
	add_option('sellegance_brands_version','1.0') or update_option('sellegance_brands_version','1.0');
...
function register_taxonomy_brands() {
	...
	$labels = array(
		'name'              => _x( 'Brands', 'taxonomy general name','sellegance' ),
		'singular_name'     => _x( 'Brand', 'taxonomy singular name','sellegance' ),
		'search_items'      => __( 'Search Brands','sellegance'  ),
		'all_items'         => __( 'All Brands','sellegance'  ),
		'parent_item'       => __( 'Parent Brand','sellegance'  ),
		'parent_item_colon' => __( 'Parent Brand:','sellegance'  ),
		'edit_item'         => __( 'Edit Brand','sellegance'  ),
		'update_item'       => __( 'Update Brand' ,'sellegance' ),
		'add_new_item'      => __( 'Add New Brand','sellegance'  ),
		'new_item_name'     => __( 'New Brand Name','sellegance'  ),
		'menu_name'         => __( 'Brands','sellegance'  ),
	);
...
function sellegance_brands_thumbnail_id($term_id)
{
	global $wpdb;
	
	return (int)$wpdb->get_var( "SELECT meta_value FROM {$wpdb->prefix}sellegance_brands where brands_term_id = $term_id");
...
function brands_admin_custom_columns_title($columns)
{
	$columns = array(	
		...
		"thumb" => __('Thumbnail','sellegance'),
		"name" => __( "Name",'sellegance'),
		"description"=> __( "Description",'sellegance'),
		"slug" => __( "Slug",'sellegance' ),
		"posts" => __( "Count",'sellegance' ),
	);
		...
		jQuery(function($){
			...
			$(document).on( 'click', '.upload_image_button', function( event ){
				...
				file_frame = wp.media.frames.downloadable_file = wp.media({
					title: '<?php _e( 'Choose an image', 'sellegance' ); ?>',
					button: {
						text: '<?php _e( 'Use image', 'sellegance' ); ?>',
					},
					multiple: false
				});
				...
				file_frame.on( 'select', function() {
					...
					$('#sellegance_brand_logo').val( attachment.id );				
...
?>
	...
	<div class="form-field">
		<label><?php _e( 'Thumbnail', 'sellegance' ); ?></label>
		...
		<div style="line-height:60px;">
			...
			<input type="hidden" id="sellegance_brand_logo" name="sellegance_brand_logo" value="" /> 
...
function brands_admin_edit_form_fields($vals,$p2){
	...
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php _e( 'Thumbnail','sellegance' ); ?></label></th>
		<td>
	<div class="brand_logo">
		<div id="product_cat_thumbnail" style="float:left;margin-right:10px;"><img src="<?php echo $image?>" width="60" height="60" /></div>
		<div style="line-height:60px;">
		<input type="hidden" id="sellegance_brand_logo" name="sellegance_brand_logo" value="<?php echo $attach_id?>" /> 
		<button type="submit" class="upload_image_button button"><?php _e( 'Upload/Add image', 'woocommerce' ); ?></button>
		<button type="submit" class="remove_image_button button"><?php _e( 'Remove image', 'woocommerce' ); ?></button>
		</div>
	</div>
...
function set_sellegance_brand_logo($params = array())
{
	global $wpdb;
	...
	$count = $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}sellegance_brands where brands_term_id = $term_id");
	
	if($count>0):
		
		$wpdb->query("update {$wpdb->prefix}sellegance_brands set meta_value = '$attach_id' where brands_term_id = $term_id and meta_key = '$meta_key'");
	
	else:
	
		$wpdb->query("insert into {$wpdb->prefix}sellegance_brands(brands_term_id, meta_key, meta_value) values($term_id, '$meta_key', '$attach_id')");	
	
	endif;
		
}

function sellegance_brands_fields_save($term_id, $tt_id, $taxonomy)
{
	if(isset($_POST['sellegance_brand_logo'])):
	
		set_sellegance_brand_logo(array('term_id' => $term_id, 'attach_id' => $_POST['sellegance_brand_logo']));
...
add_action('created_term', 'sellegance_brands_fields_save', 10,3 );
add_action('edit_term', 'sellegance_brands_fields_save', 10,3 );
