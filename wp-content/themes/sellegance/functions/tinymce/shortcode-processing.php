...
function tdl_container( $atts, $content = null ) {
	...
	return '<div class="tld_container">' . do_shortcode($content) .'</div>';
}
...
function tdl_button($atts, $content = null) {  
	extract(shortcode_atts(array("size" => 'btn-me', "url" => '#', "text" => 'Button Text'), $atts));
	$button_open_tag = '<a class="button btn btn-default '. $size .'"';
	/*switch ($size) {
		case 'small' :
			$button_open_tag = '<a class="button btn btn-default btn-sm"';
			break;
		case 'medium' :
			$button_open_tag = '<a class="button btn btn-default btn-me"';
			break;
		case 'large' :
			$button_open_tag = '<a class="button btn btn-default btn-lg"';
			break;	
	}*/
...
function tdl_icon($atts, $content = null) {
	...
	$border = '';
	return ''.$url_before.'<i class="fa '. $size . ' ' . $image . ' ' . $style . '">' . $border . '</i>'.$url_after.'';
}
	...
	(function($){
		...
		function custom_on_sale_products_UpdateSliderHeight(args) {
			...
			/* update height of the first slider */
				...
				setTimeout(function() {
					<?php global $sellegance_op; ?>
					<?php if ($sellegance_op['product_animation'] == "productanim3") { ?>
					$('li.productanim3').each(function() {
								var productImageHeight = $(this).find('.loop_product > img').height();
								$(this).find('.image_container').css('padding-bottom', productImageHeight  + 'px');
					});
...
function tdl_custom_recent_posts($atts, $content = null) {
	...
	<?php } else { //style='static' ?>

		<div class="recent_posts widget posts-widget">

			<h3 class="widget-title"><?php echo $title; ?></h3>

			<ul class="static_posts">
				<?php
				...
				if ( $recentPosts->have_posts() ) : ?>
					...
					<?php while ( $recentPosts->have_posts() ) : $recentPosts->the_post(); ?>
						...
						<li class="clearfix">
							<?php if ( has_post_thumbnail()) : ?>
							<a class="post_image" href="<?php the_permalink() ?>"><?php the_post_thumbnail('recent_posts_shortcode') ?></a>
							<?php endif; ?>
	...
	<?php
								...
								<div class="post_meta"><?php echo get_comments_number( get_the_ID() ); ?> comments</div>
			...
			<div class="items_slider portfolioitems_slider">
				<ul class="slider">
					...
					if ( $recentPosts->have_posts() ) : ?>
						...
						<?php while ( $recentPosts->have_posts() ) : $recentPosts->the_post(); 
						 $height = get_post_meta( get_the_ID(), 'tdl_page_height', true ) ? get_post_meta( $pageid, 'tdl_page_height', true ) : '350';
						 $thumb = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'large' );
						 $thumb = et_resize( $thumb[0], 500, $height, true, false );
						?>
		...
		<div class="items_slider_wrapper">
			<div class="items_slider branditems_slider">
				...
				foreach($terms as $term):
					...
					$attach_id = sellegance_brands_thumbnail_id($term->term_id);		
