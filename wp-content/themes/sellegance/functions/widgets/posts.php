...
class WP_Widget_Custom_Posts extends WP_Widget {
	...
	$widget_ops = array( 'classname' => 'posts-widget', 'description' => __('Displays your Recent or Popular Posts', 'sellegance') );
		$this->WP_Widget( 'posts_widget', __('Sellegance / Blog Posts List', 'sellegance'), $widget_ops);
	}
	
	function form($instance) {
	
		
		$instance = wp_parse_args( (array) $instance, array('title' => __('Recent Posts', 'sellegance'), 'number' => 3, 'display' => 'recent', 'followtext' => '') );
...
?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('display'); ?>">
			   <?php _e('Display Type:', 'sellegance'); ?>
			</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('followtext'); ?>">
			   <?php _e('All Blog Posts Button Text:', 'sellegance'); ?>
			</label>
...
<?php
	...
	function widget($args, $instance) {
		...
		$thumb = et_resize( $thumb[0], 80, 80, true, false );
			...
			<li class="clearfix">
				...
				<div class="post_block">
					...
					<div class="post_meta">
						...
						<?php the_time( 'jS M' ); ?> &middot; <a href="<?php echo get_permalink().'#comments'; ?>"><?php comments_number(__('No Comments', 'sellegance'), __('1 Comment', 'sellegance'), __('% Comments', 'sellegance')); ?></a>
		...
		<a class="follow-me-posts" href="<?php echo ( $sellegance_opt['blog_page'] != 0 ) ? get_permalink( $sellegance_opt['blog_page'] ) : get_post_type_archive_link( get_post_type() ); ?>" title="<?php echo $followtext; ?>"><?php echo $followtext; ?></a>
		<?php endif; ?>
