...
<form action="<?php echo esc_url( $product->add_to_cart_url() ); ?>" class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo $post->ID; ?>" data-product_variations="<?php echo esc_attr( json_encode( $available_variations ) ) ?>">
			...
			<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
				<tr>
					...
					<td class="value"><label class="variation-select"><select id="<?php echo esc_attr( sanitize_title($name) ); ?>" name="attribute_<?php echo sanitize_title($name); ?>">
						<option value=""><?php echo __( 'Choose an option', 'sellegance' ) ?>&hellip;</option>
						<?php
							if ( is_array( $options ) ) {
						...
						if ( sizeof($attributes) == $loop )
							echo '<a class="reset_variations" href="#reset">' . __( 'Clear selection', 'sellegance' ) . '</a>';
					?></td>
				</tr>
	        <?php endforeach;?>
		</tbody>
	...
	<div class="single_variation_wrap" style="display:none;">
		...
		<div class="variations_button">
			...
			<button type="submit" class="single_add_to_cart_button button alt"><?php echo apply_filters('single_add_to_cart_text', __( 'Add to cart', 'sellegance' ), $product->product_type); ?></button>
		</div>
	</div>
